...
import { formatIdentifier, toCamelCase } from './emit.js';
 ...
 */
export function generateClient(manifest: AbiManifest, clientName: string = 'Client'): string {
  const lines: string[] = [];
  ...
  lines.push(' */');
  lines.push('export type Caller = <T>(method: string, params: unknown[]) => Promise<T>;');
  lines.push('');
...
function generateMethod(method: AbiMethod, manifest: AbiManifest): string[] {
  ...
  if (method.errors && method.errors.length > 0) {
    ...
    for (const error of method.errors) {
      if (error.payload) {
        lines.push(`   * - ${error.code}: ${generateTypeRef(error.payload, manifest)}`);
      } else {
  ...
  lines.push(`  async ${methodName}(${params.join(', ')}): Promise<${nullableReturnType}> {`);
  lines.push(`    return this.call("${method.name}", [${method.params.map(p => formatIdentifier(p.name)).join(', ')}]);`);
  lines.push('  }');
