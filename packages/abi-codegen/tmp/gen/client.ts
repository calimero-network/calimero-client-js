...
export class Client {
      ...
      if (typeof variant === 'object' && 'name' in variant) {
        if ('payload' in variant) {
          ...
          if (
            ...
            convertedPayload = this.convertComplexType(variant.payload, {
              $ref: 'UpdatePayload',
            });
          }
  ...
  public async optRecord(params: {
    ...
    const convertedParams = {
      p: params.p
        ? this.convertComplexType(params.p, { $ref: 'Person' })
        : null,
    };
    const response = await this.app.execute(
      this.context,
  ...
  public async profileRoundtrip(params: {
    ...
    const convertedParams = {
      p: params.p
        ? this.convertComplexType(params.p, { $ref: 'Profile' })
        : null,
    };
    const response = await this.app.execute(
      this.context,
...
const app = {
  execute: async (_c: any, _m: string, _p?: Record<string, unknown>) => ({
    success: true,
    result: null,
  }),
} as any;
...
await client.roundtripId({
  x: '1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef',
});
await client.makePerson({
  p: {
    id: '1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef' as any,
    name: 'test',
    age: 25,
  },
});
